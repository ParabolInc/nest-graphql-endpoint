...
const addNewSelections_ = (baseSelections, selections, aliasIdx, aliasMap, isMutation) => {
    selections.forEach((selection) => {
        ...
        aliasMap[aliasedName] = selection.alias?.value ?? selectionName;
        baseSelections.push(aliasedSelection);
    });
};
const addNewOperationDefinition_ = (baseDefinitions, definition, aliasIdx, aliasMap, isMutation) => {
    ...
    addNewSelections_(baseSelections, selections, aliasIdx, aliasMap, isMutation);
};
const mergeGQLDocuments = (cachedExecParams, isMutation) => {
    ...
    cachedExecParams.forEach((execParams, aliasIdx) => {
        ...
        definitions.forEach((definition) => {
            if (definition.kind === 'OperationDefinition') {
                addNewOperationDefinition_(baseDefinitions, definition, aliasIdx, aliasMap, !!isMutation);
            }
    ...
    return { document: mergedDoc, variables: baseVariables, aliasMaps };
};
