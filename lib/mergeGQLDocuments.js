...
const getMergedSelections = (baseSelections, newSelections, definitions, aliasMap, aliasIdx, isSuffixRequired, isMutation) => {
    ...
    newSelections.forEach((newSelection) => {
        if (newSelection.kind === graphql_1.Kind.INLINE_FRAGMENT) {
            ...
            const existingInlineFragment = nextSelections.find((s) => s.kind === graphql_1.Kind.INLINE_FRAGMENT &&
        ...
        if (newSelection.kind === graphql_1.Kind.FRAGMENT_SPREAD) {
            // if it's a new fragment spread, add it, else ignore
            const existingFrag = nextSelections.find((s) => s.kind === graphql_1.Kind.FRAGMENT_SPREAD && s.name.value === newSelection.name.value);
            if (!existingFrag) {
                nextSelections.push(newSelection);
            }
            const fragDef = definitions.find((def) => def.kind === graphql_1.Kind.FRAGMENT_DEFINITION && def.name.value === newSelection.name.value);
        ...
        const existingField = isMutation
            ? undefined
            : nextSelections.find((s) => s.kind === graphql_1.Kind.FIELD &&
        ...
        const isRequestedFieldNameFree = () => {
            return !nextSelections.find((s) => {
                return s.kind === graphql_1.Kind.FIELD && requestedFieldName === (s.alias?.value ?? s.name.value);
            });
        };
        if (existingField) {
...
const addNewOperationDefinition_ = (baseDefinitions, definition, definitions, aliasIdx, isMutation, aliasMap) => {
    ...
    let matchingOp = baseDefinitions.find((curDef) => curDef.kind === graphql_1.Kind.OPERATION_DEFINITION && curDef.operation === operation);
    ...
    cachedExecParams.forEach((execParams, aliasIdx) => {
        ...
        definitions.forEach((definition) => {
            if (definition.kind === graphql_1.Kind.OPERATION_DEFINITION) {
                ...
                const existingFrag = baseDefinitions.find((def) => def.kind === graphql_1.Kind.FRAGMENT_DEFINITION && def.name.value === fragmentName);
                if (!existingFrag) {
